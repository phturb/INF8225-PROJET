...
lr = 0.005
model = create_model(env.observation_space.shape, nb_actions)
...
dqn = DQNAgent(model=model, policy=policy, nb_actions=nb_actions, memory=memory, nb_steps_warmup=32,
               target_model_update=1e-2, processor=ProcessMountainCar())
optimizer = Adam(lr=lr)
dqn.compile(optimizer, metrics=['mae'])
if TRAIN:
    dqn.fit(env, nb_steps=5000, visualize=True, verbose=2, action_repetition=4)
